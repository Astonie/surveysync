...
export default function CollectPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!navigator.onLine) {
        await db.syncQueue.add({ entityType: "response", entityId: responseId, action: "create", payload: JSON.stringify(responseData), createdAt: new Date().toISOString(), attempts: 0 });
        setSubmitted(true);
        return;
      }
        ...
        <Card className="max-w-md w-full mx-4">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              You need an account to collect data for this survey.
            </p>
      ...
      <div className="min-h-screen flex items-center justify-center bg-secondary/30">
        <Card className="max-w-md w-full mx-4">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">This survey is temporarily paused.</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-secondary/30">
        <Card className="max-w-md w-full mx-4">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              Your response has been recorded.{!isOnline && " It will be synced when you're back online."}
            </p>
            {!isOnline && pendingCount > 0 && (
              <Badge variant="warning">{pendingCount} response{pendingCount !== 1 && "s"} waiting to sync</Badge>
            )}
            <Button variant="outline" className="mt-6" onClick={() => { setSubmitted(false); setAnswers({}); setCurrentIndex(0); }}>
    ...
    <div className="min-h-screen bg-secondary/30">
      <div className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b">
        <div className="max-w-2xl mx-auto px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground mt-1">Question {currentIndex + 1} of {sortedQuestions.length}</p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
                ...
                <div className="space-y-2">
                  {currentQuestion.options.map((opt) => (
                    <label key={opt} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${answers[currentQuestion.id] === opt ? "border-primary bg-primary/5" : "hover:bg-secondary/50"}`}>
                      <input type="radio" name={currentQuestion.id} value={opt} checked={answers[currentQuestion.id] === opt} onChange={() => updateAnswer(currentQuestion.id, opt)} className="h-4 w-4 text-primary" />
                      <span>{opt}</span>
                    </label>
                  ))}
                </div>
                  ...
                  {currentQuestion.options.map((opt) => {
                    ...
                    return (
                      <label key={opt} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${selected ? "border-primary bg-primary/5" : "hover:bg-secondary/50"}`}>
                        <input type="checkbox" checked={selected} onChange={() => toggleCheckbox(currentQuestion.id, opt)} className="h-4 w-4 rounded text-primary" />
                        <span>{opt}</span>
                      </label>
                    );
                  })}
        ...
        <div className="flex justify-between mt-6">
          <Button variant="outline" onClick={() => { setError(null); setCurrentIndex(Math.max(0, currentIndex - 1)); }} disabled={currentIndex === 0}>
          ...
          {currentIndex < sortedQuestions.length - 1 ? (
            ...
            <Button onClick={handleSubmit} disabled={submitting} className="gap-2">
              ...
              {submitting ? "Submitting..." : isOnline ? "Submit" : "Submit Offline"}
            </Button>
          )}
        </div>
      </div>
